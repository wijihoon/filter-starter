...
public class XssFilter extends OncePerRequestFilter {
	 ...
	 * XssFilter의 생성자입니다.
	 *
	 * @param xssProperties XSS 탐지를 위한 설정 프로퍼티입니다.
	 */
	public XssFilter(XssProperties xssProperties) {
		this.xssProperties = xssProperties;
		this.xssPattern = xssProperties.getCompiledPattern();
	}
	...
	private boolean checkForXss(String value, String source, HttpServletResponse response) throws IOException {
		if (value != null && xssPattern.matcher(value).find()) {
	...
	private void handleXssViolation(HttpServletResponse response, ErrorCode errorCode) throws IOException {
		...
		response.getWriter().write(
			ResponseVo.error(errorCode.getMessage(), errorCode.getHttpStatus()).toString() // 에러 메시지를 JSON 형식으로 작성
		);
		response.getWriter().flush(); // 응답을 강제로 플러시하여 전송
	}
}
