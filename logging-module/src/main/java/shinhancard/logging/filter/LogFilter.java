 ...
 */
@Slf4j
@Order(Ordered.LOWEST_PRECEDENCE) // 다른 필터가 실행된 후 마지막에 실행되도록 설정
public class LogFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        return path.startsWith("/static/") || path.startsWith("/assets/") || path.endsWith(".css") || path.endsWith(".js");
    }
    ...
    private String maskSensitiveData(String body) {
        for (String field : logProperties.getSensitiveFields()) {
            body = body.replaceAll(String.format("\"%s\":\"[^\"]*\"", field), String.format("\"%s\":\"[PROTECTED]\"", field));
        }
        return body;
    }
}
